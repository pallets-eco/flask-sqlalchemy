...
def parse_version(v):
    """Take a string version and convert it to a tuple for easier
    comparison. ::
    ...
    """
    parts = v.split(".")
    # Pad the list to make sure there is three elements so that we get
    # major, minor, point comparisons that default to "0" if not given.
    # I.e. "1.2" --> (1, 2, 0)
    parts = (parts + 3 * ['0'])[:3]
