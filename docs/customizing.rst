...
.. warning::
    ...
    The session instance generated by Flask-SQLAlchemy is created by
    :class:`~sqlalchemy.orm.session.sessionmaker` and will only proxy a few, predetermined
    methods from the default Session class. Adding new session methods is not an intended
    useage of this feature.

    For more information, `see the sessionmaker documentation
    <http://docs.sqlalchemy.org/en/latest/orm/session_api.html#session-and-sessionmaker>`_
